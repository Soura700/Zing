...
router.post("/create", upload.array("images", 5), async (req, res) => {
  ...
  if (req.files.length === 0) {
    try {
      ...
      connection.query(checkUserQuery, [userId], (err, results) => {
        if (err) {
          ...
          // User exists, insert the post with only description
          const insertPostQuery =
            "INSERT INTO posts (userId, username, description) VALUES (?, ?, ?)";
          const values = [userId, username, description];
          connection.query(insertPostQuery, values, (err, result) => {
            if (err) {
              console.error("Error creating post:", err);
              res.status(500).json(err);
              ...
              res
                .status(201)
                .json({ id: result.insertId, ...req.body, images: null });
            }
          });
        }
      });
    ...
    // Images were uploaded, proceed with the existing logic

    // Sightengine API call to check for nudity, etc.
    try {
      const form = new FormData();
      req.files.forEach((file) => {
        form.append("media", fs.createReadStream(`uploads/${file.filename}`));
      });
      form.append("models", "nudity-2.0");
      form.append("api_user", "1498986529");
      form.append("api_secret", "77xk4zUxM6gJBVsftXLohCjsCKxuyzGw");
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Nudity detected in the images. 18+ content cannot be uploaded.",
        });
      }
      ...
      connection.query(checkUserQuery, [userId], (err, results) => {
        if (err) {
          ...
          // User exists, insert the post with description and images
          const insertPostQuery =
            "INSERT INTO posts (userId, username, description, image) VALUES (?, ?, ?, ?)";
          const images = req.files.map((file) => file.filename);
          const values = [
